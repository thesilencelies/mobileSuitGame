...
Writes one Tabletop Simulator card-metadata JSON per card into `json/` — 116 action cards plus 12 frames. The file's single top-level key is the `raw.githubusercontent.com` URL of that card's `AllCardImages/*.png` (URL-encoded: most card images have spaces in the filename), and the value holds TTS's fixed `name`/`description`/`gm_notes`/`tags` fields — no others can be added.
...
Run: `python generate_card_json.py [--output-dir json] [--quiet]`
...
- `json/` — per-card Tabletop Simulator metadata, one file per card, generated by `generate_card_json.py`
