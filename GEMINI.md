...
A data-driven card game toolkit that generates printable cards and Tabletop Simulator (TTS) assets from CSV data files, alongside a phone-playable playtest web engine and combat simulation suite.

## Working in this repo

**Commit and push to `master` directly** unless asked for a branch. This is a solo repo and the default "branch first" habit just adds a merge step.

## Antigravity Customizations & Skills

Skills are registered in `.agents/skills/` (and `.agents/skills.json`):

- **`weapon-power`** (`.agents/skills/weapon-power/SKILL.md`): Combat simulator analysis assessing weapon group power, isolating offence vs blocking, marginal contribution, and pairings.
- **`weapon-balance`** (`.agents/skills/weapon-balance/SKILL.md`): Paper-balance dominance audit via `weapon_dominance.py` (within-group and cross-group strictly dominated cards).
- **`playtest-ai`** (`.agents/skills/playtest-ai/SKILL.md`, references at `.agents/references/playtest-ai.md`): Standing record of playtest AI tuning, measurement protocols (`playtest.ai.arena --panel`), traps, failed levers, and open tasks. **Read it before tuning `playtest/ai/`**.
- **`playtest-engine`** (`.agents/skills/playtest-engine/SKILL.md`, references at `.agents/references/playtest-engine-notes.md`): Critical rules, card text parsing requirements, hidden-information leak prevention, rulings, and the verification stack for `playtest/`.

Hierarchical directory rules are also located at:
- `playtest/GEMINI.md` / `playtest/AGENTS.md`
- `simulation/GEMINI.md` / `simulation/AGENTS.md`
...
> [!TIP]
> Use `python3` (or `.venv/bin/python`) when running scripts in this repo.
...
Credentials are resolved in order: `$RULES_DRIVE_CREDENTIALS`, then `~/.config/mobilesuit/drive_service_account.json` (a service account — headless, preferred; the Drive file must be *shared with its e-mail as Editor* or the update 404s), then `~/.config/mobilesuit/client_secret.json` (an OAuth desktop client — opens a browser once, caches a refresh token in `~/.config/mobilesuit/drive_token.json`). Running it with none configured prints the full one-time setup and exits 2. Needs `google-api-python-client google-auth google-auth-oauthlib`.
The VS Code task **create and publish rules** runs `create rules` then `upload rules` in sequence; plain `create rules` still builds without publishing.
...
### weapon_dominance.py
Finds cards that are strictly better than other cards for the same printed effect, and prints each weapon group's profile (zone coverage, range span, initiative/movement spread, attack and block per card) so a per-card gap can be read against how similar the two groups otherwise are. Three sections: `within` (same group, same effect, different initiative/movement), `cross` (a card from another group dominating on attack/block/range/initiative/movement — clustered, so a card beaten by several others is one entry rather than several), `profiles`. Zone placement is never treated as a difference (attack/block values compare as sorted shapes). Only cards printing identical text and persistence are compared — text is whitespace-normalised first, since a stray double space used to hide real duplicates. Damage type is printed on every pair but never gates a comparison. Backs the `weapon-balance` skill in `.agents/skills/`.
...
- `json/` — `cards.json` and `decks.json`, Tabletop Simulator metadata for cards, tiles (tokens), figurines, and decks, generated by `generate_card_json.py` and `generate_deck_json.py`
- `RulesImages/` — annotated rules-reference card PNGs (one per card type), rendered by `generate_all_decks.py`
- `rules/` — `rules.tex` and its sources. `rules.pdf` is built here but **not tracked**; publish it with `upload_rules.py`
